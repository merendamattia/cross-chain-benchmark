...
import { meson, MesonContract, ERC20Contract } from '@mesonfi/base'
...
export class MesonClient {
  ...
  protected _tokens: { tokenIndex: number, addr: string }[] = []

  static async Create(mesonInstance: MesonContract, swapSigner?: SwapSigner) {
  ...
  constructor(mesonInstance: any, shortCoinType: string) {
    ...
    this.#mesonInstance = mesonInstance as MesonContract
  ...
  getTokenContract(tokenAddr: string, provider = this.provider) {
    return this.getContractInstance(tokenAddr, meson.ABI.ERC20, provider) as ERC20Contract
  }
  ...
  async getTokenBalance(addr: string, tokenIndex: number) {
    if (this.isCoreToken(tokenIndex)) {
      ...
      return meson.formatBalance(rawValue, this.coreDecimals)
    }
    ...
    return meson.formatBalance(rawValue, decimals)
  }

  async getAllowance (addr: string, tokenIndex: number) {
    if (this.isCoreToken(tokenIndex)) {
      return meson.formatBalance(BigNumber.from(2).pow(128).sub(1))
    }
    const tokenAddr = await this.#asyncGetTokenAddr(tokenIndex, { from: addr })
    const tokenContract = this.getTokenContract(tokenAddr)
  ...
  async getBalanceInPool(owner: string, tokenIndex: number) {
    ...
    return meson.formatBalance(rawValue, 6)
  }
  ...
  async inContractTokenBalance(tokenIndex: number, ...overrides: [CallOverrides?]) {
    if (this.isCoreToken(tokenIndex)) {
      ...
      return meson.formatBalance(rawValue, 18)
    }
