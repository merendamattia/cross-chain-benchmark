...
export class Swap implements SwapData {
  ...
  private _makeFullSalt(data: SwapData): string {
    ...
    if (saltHeader) {
      ...
      if (this.swapForCoreToken) {
        if (this._newFormat) {
          const saltData1 = Swap.compressFixedPrecision((coreTokenPrice || 0) * 1e4)
          if (saltData1.length > 4) {
            throw new Error('Invalid coreTokenPrice; overflow')
          }
          const saltData2 = data.amountForCoreToken === 'all'
            ? 'ffff'
            : Swap.compressFixedPrecision(BigNumber.from(data.amountForCoreToken || 0).div(1e3))
        ...
        const saltData2 = Swap.compressFixedPrecision(amountToShare)
        if (saltData2.length > 4) {
  ...
  get usePoints(): boolean {
    return this.willWaiveFee && (this.inToken < 64 || this.inToken > 191) && (parseInt(this.salt[3], 16) & 1) === 1
  }

  get pointsUsed(): BigNumber {
    if (!this.usePoints) {
      return BigNumber.from(0)
    } else if (this.inToken < 64) {
      return this._serviceFee.div(1e4)
    } else if (this.inToken > 191) {
      const tokenPrice = this.swapForCoreToken ? this.coreTokenPrice : this._coreTokenPrice
      return this._serviceFee.mul(Math.round(tokenPrice * 1e6)).div(1e6).div(1e4)
    }
    return BigNumber.from(0)
  }
  ...
  get amountForCoreToken(): BigNumber {
    if (!this.swapForCoreToken) {
      ...
      return BigNumber.from(Swap.decompressFixedPrecision(d)).mul(1e3)
    }
  ...
  get coreTokenPrice(): number {
    if (!this.swapForCoreToken) {
      ...
      return this._coreTokenPrice
    }
    return parseInt(this.salt.slice(6, this._splitPos), 16) / this._pFactor
  }

  get _coreTokenPrice(): number {
    return Number(Swap.decompressFixedPrecision('0x' + this.salt.slice(6, 10))) / 1e4
  }
  ...
  get amountToShare(): BigNumber {
    ...
    return BigNumber.from(Swap.decompressFixedPrecision('0x' + this.salt.slice(10, 14)))
  }

  static compressFixedPrecision(value: BigNumberish, precision = 3): string {
  ...
  static decompressFixedPrecision(hex: string, precision = 3): BigInt {
