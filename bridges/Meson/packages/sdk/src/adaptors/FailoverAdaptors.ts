...
function extendToFailoverAdaptor<ClassAdaptor extends AdaptorConstructor>(Adaptor: ClassAdaptor): ClassAdaptor {
  return class FailoverAdaptor extends Adaptor {
        ...
        get(target, prop: string) {
          ...
          return (...args) => {
            ...
            return new Promise(async (resolve, reject) => {
              try {
                resolve(await meson.timeout(result, that.#timeout))
                return
              } catch (e) {
                errors.push(e)
              }
              ...
              while (adaptors.length) {
                const current = adaptors.pop()
                try {
                  resolve(await meson.timeout(current.client[prop](...args), that.#timeout))
                  return
                } catch (e) {
                  errors.push(e)
                }
    ...
    async send(method, params) {
      return await meson.timeout(this._send(method, params), 30_000)
    }
    ...
    private async _send(method, params) {
      ...
      while (adaptors.length) {
        const current = adaptors.pop()
        try {
          return await meson.timeout(current.send(method, params), this.#timeout)
        } catch (e) {
          errors.push(e)
        }
      }
