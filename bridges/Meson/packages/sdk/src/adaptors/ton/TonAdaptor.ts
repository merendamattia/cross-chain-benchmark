...
export default class TonAdaptor implements IAdaptor {
  ...
  protected _wrapTonTx(tx: Trace) {
    ...
    const { decoded_op_name, decoded_body } = transaction.in_msg

    let messageList
    if (decoded_op_name === 'wallet_signed_v3' || decoded_op_name === 'wallet_signed_v4') {
      messageList = decoded_body.payload.map(a => a.message)
    } else if (decoded_op_name === 'wallet_signed_external_v5_r1') {
      messageList = decoded_body.actions.map(a => a.msg)
    }

    const parsed = messageList?.map(msg => {
      switch (msg.sum_type) {
        case 'MessageInternal': {
          const data = msg.message_internal.body.value
