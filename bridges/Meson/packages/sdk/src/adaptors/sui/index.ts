    ...
    get(target, prop: string) {
      if (prop === 'address') {
        ...
        return async (target, getArguments) => {
          ...
          const { arguments: args = [], typeArguments = [] } = getArguments(transaction, metadata)
          const payload = { target, arguments: args, typeArguments }
          transaction.moveCall(<any>payload)
          return await (<SuiWallet>adaptor).sendTransaction(transaction)
        }
        ...
        if (['view', 'pure'].includes(method.stateMutability)) {
          return async (...args) => {
            ...
            if (prop === 'getShortCoinType') {
              ...
              const result = await getDynamicFieldValue(storeG.posted_swaps, { type: 'vector<u8>', value: _arrayify(swap.encoded + 'ff') })
              if (!result) {
                return { exist: false }
              }
              ...
              const storeG = await getStoreG()
              const swap = Swap.decode(args[0])
              const swapId = meson.getSwapId(swap.encoded, args[1])
              const result = await getDynamicFieldValue(storeG.locked_swaps, { type: 'vector<u8>', value: _arrayify(swapId) })
              if (!result) {
          ...
          return async (...args) => {
            ...
            if (prop === 'transfer') {
              ...
              const [coin] = transaction.splitCoins(transaction.object(coins[0].coinObjectId), [value])
              ...
              payload.arguments = [
                transaction.object(metadata.adminCap),
                transaction.pure.u8(tokenIndex),
                transaction.object(metadata.storeG),
              ]
              ...
              payload.typeArguments = [tokenAddr]
              payload.arguments = [
                transaction.pure.u64(BigNumber.from(args[0]).toHexString()),
                transaction.pure.u64(poolIndex),
              ...
              payload.arguments = [transaction.object(args[0]), transaction.object(metadata.storeG)]
              ...
              payload.arguments = [
                transaction.object(metadata.adminCap),
                transaction.pure.u64(BigNumber.from(amount).toHexString()),
                transaction.pure.u64(toPoolIndex),
                transaction.object(metadata.storeG),
              ]
            } else {
              const swap = Swap.decode(args[0])
              if (prop === 'postSwapFromInitiator') {
                ...
                payload.typeArguments = [tokenAddr]
                payload.arguments = [
                  _vectorize(transaction, swap.encoded),
                  _vectorize(transaction, postingValue.substring(0, 42)), // initiator
                  transaction.pure.u64(`0x${postingValue.substring(42)}`), // pool_index
                  transaction.makeMoveVec({ elements: coinList.map(obj => transaction.object(obj.coinObjectId)) }),
                ...
                payload.arguments = [
                  _vectorize(transaction, swap.encoded),
                  transaction.pure.u64(args[1].toString()),
                  transaction.object(metadata.storeG),
                ]
                ...
                payload.arguments = [
                  _vectorize(transaction, swap.encoded),
                  _getCompactSignature(transaction, r, yParityAndS),
                  _vectorize(transaction, recipient.substring(0, 42)),
                  transaction.pure.bool(depositToPool),
                  transaction.object(metadata.storeG),
                  transaction.object('0x6'),
                ]
                ...
                payload.arguments = [
                  _vectorize(transaction, swap.encoded),
                  _vectorize(transaction, initiator),
            ...
            return await (<SuiWallet>adaptor).sendTransaction(transaction, options)
          }
        }
      }
      return target[prop]
...
function _arrayify(hex: string) {
  return Array.from(utils.arrayify(hex))
}

function _vectorize(transaction: SuiTransaction, hex: string) {
  return transaction.pure('vector<u8>', _arrayify(hex))
}

function _getCompactSignature(transaction: SuiTransaction, r: string, yParityAndS: string) {
  return _vectorize(transaction, r + yParityAndS.replace(/^0x/, ''))
}
