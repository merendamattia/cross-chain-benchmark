...
const { meson } = require('@mesonfi/base')
...
exports.addSupportedTokens = async function addSupportedTokens(tokens, { network, wallet }) {
  const mesonInstance = adaptors.getContract(network.mesonAddress, meson.ABI.Meson, wallet)
...
exports.deposit = async function deposit(symbol, amount, { network, wallet }) {
  ...
  const value = meson.toSwapValue(amount)
  if (allowance.value.lt(value)) {
    console.log(`Approving...`)
    const toApprove = meson.toSwapValue('1000000000') // 1B
...
exports.withdraw = async function withdraw(symbol, amount, { network, wallet }) {
  ...
  // const gasLimit = await mesonInstance.estimateGas.withdraw(meson.toSwapValue(amount), poolTokenIndex)
  const tx = await mesonInstance.withdraw(meson.toSwapValue(amount), poolTokenIndex)
  await tx.wait(1)
  return tx
}
...
exports.withdrawServiceFee = async function withdrawServiceFee(symbol, amount, { network, wallet }) {
  ...
  const tx = await mesonInstance.withdrawServiceFee(tokenIndex, meson.toSwapValue(amount), 1)
  await tx.wait(1)
  return tx
}
...
exports.send = async function send(symbol, amount, recipient, { network, wallet }) {
  ...
  const tx = await mesonClient.transferToken(tokenIndex, recipient, meson.toSwapValue(amount))
  await tx.wait(1)
  return tx
}
